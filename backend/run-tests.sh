...
main() {
    ...
    if [ $# -eq 1 ]; then
        local service=$1
        case $service in
            "auth-service"|"device-service"|"telemetry-service"|"event-service"|"command-service"|"api-gateway")
                run_service_tests "$service"
                ;;
            *)
                print_error "Неизвестный сервис: $service"
                print_status "Доступные сервисы: auth-service, device-service, telemetry-service, event-service, command-service, api-gateway"
                exit 1
                ;;
        esac
    else
        # Запуск тестов во всех сервисах
        local services=("auth-service" "device-service" "telemetry-service" "event-service" "command-service" "api-gateway")
